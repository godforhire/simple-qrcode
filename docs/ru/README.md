...
```bash
composer require godforhire/simple-qrcode
```

Либо добавьте пакет `godforhire/simple-qrcode` в раздел `require` файла `composer.json`:
...
Добавьте строчку 
```
godforhire\QrCode\QrCodeServiceProvider::class
``` 
...
Добавьте строчку 
```
'QrCode' => godforhire\QrCode\Facades\QrCode::class
``` 
